  ...
  if (curConfig.smartthings.sendNotification
    ...
    if (messages.length > 0) {
      const smsText = messages.filter((m) => {
        ...
        return notification && (notification[m.code] === 'both' || notification[m.code] === 'sms');
      }).map((m) => m.code).join(',');
      if (smsText) {
        addSMSNotification1(smsText, curConfig);
      }
      const promises = [];
      messages.forEach((m) => {
        const notification = curConfig.notifications[m.code];
        if (notification && (
          notification[m.code] === 'both'
            || notification[m.code] === 'push')) {
