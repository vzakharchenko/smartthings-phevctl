...
async function smartthingsAddDevice(device) {
  ...
  const status = await addDeviceToSmartThings(
    smartthings.shard,
    smartthings.appId,
...
async function smartthingsAllDevice() {
  const { smartthings } = readConfig();
  const status = await getAllDevicesFromSmartThings(
    smartthings.shard,
    smartthings.appId,
    smartthings.appSecret,
  );
...
async function smartthingsAllCloudDevice() {
  const { smartthings } = readConfig();
  const status = await getAllCloudDevicesFromSmartThings(
    smartthings.shard,
    smartthings.appId,
    smartthings.appSecret,
  );
...
async function smartthingsDeleteDevice(deviceId) {
  const { smartthings } = readConfig();
  const status = await deleteDeviceFromSmartThings(
    smartthings.shard,
    smartthings.appId,
    smartthings.appSecret,
    deviceId,
...
async function smartthingsUpdateDevice(device) {
  ...
  if (device.updatable && rc.smartthings.useSmartthings) {
    ...
    if (value) {
      ...
      await updateSmartThingsDevice(
        rc.smartthings.shard,
        rc.smartthings.appId,
        rc.smartthings.appSecret,
        device.id,
        v,
        v2,
      );
